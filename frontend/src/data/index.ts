// AUTO-GENERATO da tools/sync_data.py — non modificare a mano

import formations from './formations.json';
import counterEngine from './counterEngine.json';
import scoutTips from './scoutTips.json';
import playerRoles from './playerRoles.json';
import positionGuide from './positionGuide.json';
import metaTactics from './metaTactics.json';
import specialAbilities from './specialAbilities.json';
import mentors from './mentors.json';
import mentorTactics from './mentorTactics.json';
import trainingGuide from './trainingGuide.json';
import realTeams from './realTeams.json';
import faq from './faq.json';
import abbreviations from './abbreviations.json';
import careerPaths from './careerPaths.json';
import myPlaybook from './myPlaybook.json';
import setPiece from './setPiece.json';
import battleCards from './battleCards.json';
import gameGuide from './gameGuide.json';
import formationLab from './formationLab.json';
import realTactics from './realTactics.json';

export const FORMATIONS = formations as any[];
export const COUNTER_ENGINE = counterEngine as any[];
export const SCOUT_TIPS = scoutTips as any[];
export const PLAYER_ROLES = playerRoles as any[];
export const POSITION_GUIDE = positionGuide as any[];
export const META_TACTICS = metaTactics as any[];
export const SPECIAL_ABILITIES = specialAbilities as any[];
export const MENTORS = mentors as any[];
export const MENTOR_TACTICS = mentorTactics as any[];
export const TRAINING_GUIDE = trainingGuide as any[];
export const REAL_TEAMS = realTeams as any[];
export const FAQ = faq as any[];
export const ABBREVIATIONS = abbreviations as any[];
export const CAREER_PATHS = careerPaths as any[];
export const MY_PLAYBOOK = myPlaybook as any[];
export const SET_PIECE = setPiece as any[];
export const BATTLE_CARDS = battleCards as any[];
export const GAME_GUIDE = gameGuide as any[];
export const FORMATION_LAB = formationLab as any[];
export const REAL_TACTICS = realTactics as any[];

export { APP_META } from './appMeta';
